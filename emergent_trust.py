...
def update_trust(G, iterations):
    for interaction in iterations:
        ...
        #  TODO осмыслить формулу для расчета доверия
...
def draw_graph(agents, trusts, interactions=100):
    ...
    for u in G.nodes():
        for v in G.nodes():
            if not v == u:
                G[u][v]['trust'] = u.get_trust_score_by_id(v.ID)
                print(u, v, u.get_interactions_by_id(v.ID))
                edge_widths.append(u.ID*10) #TODO заменить при наличии интераптионов
    ...
    edge_colors = [(1 - G[u][v]['trust'], G[u][v]['trust'], 0) for u, v in G.edges()]
...
agents = [Agent(1),
          Agent(2, "123", 0.95), Agent(3, "qwe", 0.1)]
trusts = [Trust(agents[1], agents[0], 0.5),
